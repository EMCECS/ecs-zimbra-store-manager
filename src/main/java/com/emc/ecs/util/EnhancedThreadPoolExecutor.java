...
public class EnhancedThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    protected void beforeExecute(Thread t, Runnable r) {
        if (threadsToKill.tryAcquire()) {
            EcsLogger.debug("terminating thread due to shrinking pool");
        ...
        synchronized (pauseLock) {
            if (paused) {
                EcsLogger.debug("thread has been paused");
                try {
                    pauseLock.wait();
                    EcsLogger.debug("thread has been resumed");
                } catch (InterruptedException e) {
                    EcsLogger.warn("interrupted while paused; might be shutting down");
                    workDeque.addFirst(r);
    ...
    protected void afterExecute(Runnable r, Throwable t) {
        ...
        if (shutdownWhenIdle && aTasks == 0 && uTasks == 0 && workDeque.isEmpty()) {
            EcsLogger.info("shutting down pool because it is idle and shutdownWhenIdle is true");
            shutdown();
        }
    ...
    public void blockingSubmit(Runnable task) {
        while (true) {
            ...
            synchronized (submitLock) {
                ...
                if (this.isShutdown()) throw new IllegalStateException("executor is shut down");
                try {
                    EcsLogger.debug("task queue is full; waiting until space is available");
                    submitLock.wait();
                    EcsLogger.debug("task queue has space; resubmitting task");
                } catch (InterruptedException e) {
                    EcsLogger.warn("interrupted while waiting to submit a task", e);
                }
            }
        }
    }
    ...
    static class ExceptionHandler implements Thread.UncaughtExceptionHandler {
        ...
        public void uncaughtException(Thread t, Throwable e) {
            if (!(e instanceof PoolTooLargeException)) {
                EcsLogger.warn("uncaught exception from task", e);
